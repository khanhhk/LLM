...
from functions import get_tool_schema, calculate_fixed_monthly_payment, calculate_future_value
...
VAST_PORT="29593"
deepseek_client = OpenAI(api_key="", base_url=f"http://{VAST_IP_ADDRESS}:{VAST_PORT}/v1")
deepseek_cloud_client = OpenAI(api_key=DEEP_SEEK_API_KEY, base_url="https://api.deepseek.com")
...
def chat_complete(messages=(), model="gpt-4o-mini", raw=False):
    ...
    response = called_client.chat.completions.create(
        model=model,
        messages=messages
    )
    if raw:
